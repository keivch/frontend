...
const ContentPassword = ({ title, items, onViewMore, onEdit, onHandleClick }) => {
      ...
      <ul className="space-y-4">
        {filteredItems.map((item) => (
          <li key={item.id} className="flex justify-between items-center bg-gray-100 p-4 rounded-md shadow-sm">
            {/* Nombre del elemento */}
            <span className="text-lg">{item.correo}</span>
            <span className="text-lg">{item.tipo}</span>
